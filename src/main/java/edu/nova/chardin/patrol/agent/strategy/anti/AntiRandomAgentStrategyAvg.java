...
public class AntiRandomAgentStrategyAvg implements AgentStrategy {
  ...
  public EdgeId choose(final AgentContext context) {
    ...
    final ImmutableMap<EdgeId, DoubleSummaryStatistics> coveredVertexEdgeScores;
    ...
    chosenEdge = context.getIncidientEdgeIds().stream()
            .map(edgeId -> {
              ...
              final Optional<DoubleSummaryStatistics> coveredVertexScoreStatistics = Optional.ofNullable(coveredVertexEdgeScores.get(edgeId));
              final double coveredVertexScore = coveredVertexScoreStatistics.map(DoubleSummaryStatistics::getAverage).orElse(1.0);
